...
export default function ItensCart () {
  ...
  async function cepPrice () {
    if (cep) {
      const response = await axios.get(`https://zs5utiv3ul.execute-api.us-east-1.amazonaws.com/dev/freight/${cep}`)
      if (response.data.freight) {
        setPriceFrete(response.data.freight)
        setOpen(false)
      } else {
        setOpen(true)
        setTimeout(() => {
        ...
        <div>
          ...
          <div className="container-cart">
            ...
            <div className="frete">
              ...
              />
              <button className="frete-ok" onClick={(e) => cepPrice(cep)}>
                Calcular
              </button>
            </div>
            { open &&
            ...
            <div className="cart-price">
              <span>
                 Frete: { pricefrete !== 0
                  ? parseInt(pricefrete.replace('.', ',')).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
                  : 'R$ 0,00'}
              </span>
              <span>
